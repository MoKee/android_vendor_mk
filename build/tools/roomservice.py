...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from MoKee Open Source Github (http://github.com/MoKee)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:MoKee+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('MoKee/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: MoKee/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "mokee", "name": "MoKee/%s" % repo_name })
