#!/bin/bash
#
# Copyright (C) 2016-2020 The MoKee Open Source Project
...
function setup_vendor() {
    ...
    export ANDROIDBP="$MOKEE_ROOT"/"$OUTDIR"/Android.bp
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        ...
        printf " * Copyright (C) 2019-$YEAR The MoKee Open Source Project\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The MoKee Open Source Project\n" >> $1
    fi
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$MOKEE_ROOT"/vendor/mokee/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [[ "$FULLY_DEODEXED" -ne "1" && "${VENDOR_REPO_FILE}" =~ .(apk|jar)$ ]]; then
            ...
            if [ -f "$TMPDIR/classes.dex" ]; then
                ...
                zip -gjqX "${VENDOR_REPO_FILE}" "$TMPDIR/classes"*
                rm "$TMPDIR/classes"*
