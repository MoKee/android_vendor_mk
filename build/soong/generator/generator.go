...
func init() {
	android.RegisterModuleType("mokee_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = mokeeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = mokeeExpandVariables(ctx, dep_file)
	...
	cmd := mokeeExpandVariables(ctx, String(g.properties.Cmd))
